...
namespace AoC2020
{
    public class Day17
    {
        ...
        public static Int64 Day17b(string[] input)
        {
            var inputSize = input[0].Length;
            var size = input[0].Length + 2 * 6;
            var grid = new bool[size, size, size, size];

            var x = (size - inputSize) / 2;
            var y = (size - inputSize) / 2;
            var z = size / 2;
            var w = (size - inputSize) / 2;
            foreach (var line in input)
            {
                if (line == "") break;
                x = (size - inputSize) / 2;
            ...
            {
                setCount = 0;
                var newGrid = new bool[size, size, size, size];
                for (w = 0; w != size; ++w)
                {
                    for (z = 0; z != size; ++z)
                    {
                                ...
                                for (var dx = -1; dx != 2; ++dx)
                                {
                                    ...
                                    for (var dy = -1; dy != 2; ++dy)
                                    {
                                        ...
                                        for (var dz = -1; dz != 2; ++dz)
                                        {
                                            ...
                                            {
                                                if (w + dw < 0) continue;
                                                if (w + dw >= size) continue;
                                                if (dx == 0 && dy == 0 && dz == 0 && dw == 0) continue;
                                                if (grid[x + dx, y + dy, z + dz, w + dw]) ++neightbours;
                                            }
                                        }
                                    }
                                }
                                if (grid[x, y, z, w])
                                    newGrid[x, y, z, w] = (neightbours == 2 || neightbours == 3);
                                else
                                    newGrid[x, y, z, w] = (neightbours == 3);
                                if (newGrid[x, y, z, w]) ++setCount;
                                //Console.Write((newGrid[x, y, z] ? '#' : '.'));
                            }
                            //Console.WriteLine("");
                        }
